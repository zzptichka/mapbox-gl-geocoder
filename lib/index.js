 ...
 * @param {Function} [options.asyncGeocoder] Same as localGeocoder but returns a promise. Can be used to supplement geocode from external API such as OpenStreetMap Nominatim.
 * @example
...
MapboxGeocoder.prototype = {
  ...
  _geocode: function(searchInput) {
    ...
    const mapboxRequest = this.mapboxClient.geocodeForward(searchInput, this.options).then(function (response) {
      var res = []
      if (response.status == '200' && response.entity) {
        if(response.entity.features) {res = response.entity.features;}
      }
      return res;
    })

    var customRequest;
    if (this.options.asyncGeocoder) {
      customRequest = this.options.asyncGeocoder(searchInput);
    }
    else{
    ...
    Promise.all([mapboxRequest, customRequest]).then(function(values) {
      ...
      // concat results from all 3 sources
      res.features = localGeocoderRes.concat(values[0],values[1]);
